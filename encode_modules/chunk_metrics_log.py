...
This module owns the ``ChunkMetricsLog`` class (per-chunk record/emission +
per-file ``aggregate_summary`` rollup). The process-wide singleton, the
module-level shims (``init_chunk_metrics_log`` / ``record_chunk_metrics`` /
``update_chunk_quality`` / ``aggregate_summary`` / ``get_log``), and
``build_summary_for_history_record`` live in the sibling
``chunk_metrics_singleton`` (split out to stay under the 500-line cap) and
are re-exported here so existing importers keep working unchanged. Tests
...
__all__ = ["ChunkMetricsLog", "compute_size_percent",
           "build_summary_for_history_record",
...
# ----- Singleton + module-level shims (re-exported) -------------------------
# The singleton ``_log``, its shims, and ``build_summary_for_history_record``
# moved into ``chunk_metrics_singleton`` to keep this module under the
# 500-line cap. Re-exported here so the many existing importers
# (``from .chunk_metrics_log import record_chunk_metrics`` etc. in
# chunk_worker / chunk_recovery / encode_serial / encode_parallel /
# encode_resumable / reporting / history_state) and the tests that call
# ``chunk_metrics_log._reset_for_tests`` keep working unchanged. The import
# sits at the BOTTOM (not the top) because chunk_metrics_singleton imports
# ``ChunkMetricsLog`` from this module — defining the class first avoids a
# circular-import failure at module load.
from .chunk_metrics_singleton import (  # noqa: E402
    aggregate_summary,
    build_summary_for_history_record,
    get_log,
