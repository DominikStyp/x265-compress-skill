...
def print_threshold_abort_block(workdir: Path, abort_reason: str) -> None:
    ...
    print(f"{Y}  The projected file would be bigger than your size limit, so{R}")
    print(f"{Y}  this source probably won't shrink much at this quality (it{R}")
    print(f"{Y}  may already be efficiently compressed). Details:{R}")
