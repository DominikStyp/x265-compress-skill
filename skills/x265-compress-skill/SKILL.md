...
- **Interactive pause/resume per slot** (htop-style). While the encode is running, keypresses control individual slots without killing the run:
  ...
  Paused slots show `[PAUSED]` next to the chunk row and burn zero CPU. Suspension signals the ffmpeg PID directly — Win32 `NtSuspendProcess` on Windows, `SIGSTOP`/`SIGCONT` on macOS/Linux — so x265's RAM state (lookahead, reference frames, RC) is preserved bit-for-bit; resume picks up at the exact same frame with no quality loss. The keys need an interactive terminal (a TTY): they work in single-file mode and inside `run_queue.py` when you launch it in a real terminal (stdin is inherited through the launcher → script → python). If you run detached/headless (`nohup`, `&`, output redirected to a log, cron), there's no TTY, so the keys are off — use the file-based fallback below.
  **Pause without a keyboard (`PAUSE` file)** — create a `PAUSE` file in the workdir to suspend **every** active slot, and delete it to resume them all. The encoder polls for it ~2×/second, so it also re-pauses freshly-started chunks while the file is present. This is the headless/over-SSH counterpart to the Space/1-9 keys (`encode_resumable.py` prints the workdir at startup). Note: while the `PAUSE` file exists the encode stays suspended — including at the point it would otherwise finish — so remove it to let the run complete (or use `FINISH` for a graceful stop-after-current-chunk).
